...
const VerifyEmailSection = () => {
    const {email, setProgress, progress} = useContext(RegisterContext);
    ...
    useEffect(() => {
        if (progress.position === RegisterProgress.VerifyEmail.position) {
    ...
    return (
        <Wrapper>
            <Content>
                <Title type="s_28_34">Verify your email address</Title>
                <Description type="r_14_17">
                    Enter the 6-digits OTP sent to your email address
                    <Text type="s_14_17">{`\n${email} `}</Text>
                    <ChangeEmail suppressHighlighting={true} onPress={onChangeEmailPressed} type="s_14_17">
